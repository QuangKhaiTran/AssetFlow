  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div id="qr-code-grid-for-pdf" className="absolute -left-[9999px] top-auto w-[210mm] bg-white p-[10mm]">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '5mm', pageBreakInside: 'auto' }}>
            {assets.map((asset) => (
                <div key={asset.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', border: '1px solid #eee', padding: '3mm', borderRadius: '4px', pageBreakInside: 'avoid' }}>
                    <QRCodeComponent value={asset.id} size={100} />
                    <p style={{ marginTop: '3mm', fontSize: '8px', textAlign: 'center', fontWeight: 'bold' }}>{asset.name}</p>
                    <p style={{ fontSize: '6px', textAlign: 'center', fontFamily: 'monospace' }}>{asset.id}</p>
                </div>
            ))}
        </div>
      </div>
